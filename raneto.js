...
var raneto = {
    ...
    getPages: function (activePageSlug) {
        ...
        var page_sort_meta = raneto.config.page_sort_meta || '',
            ...
            providers = glob.sync(raneto.config.provider_dir + '*'),
            filesProcessed = [];
        ...
        providers.forEach(function (providerPath) {
            var providerUrl = fs.readFileSync(providerPath);
            var client = request.createClient('http://' + providerUrl);
            var get_content = Q.defer();
            client.get('api', function (err, res, body) {
                if(err) {
                    get_content.reject(err);
                }
                get_content.resolve(body);
            });
            promises.push(get_content.promise);
        });
            ...
            .then(function (data) {
                data.forEach(function(response) {
                    response.pages.forEach(function(page) {
                        filesProcessed.push(page);
                    });
                });

                filesProcessed = _.sortBy(filesProcessed, function (cat) {
                    return cat.sort;
                });
